...
function fixture() {
  const sourcePlan = signedPlan("profile-drift-source");
  const driftPlan = signedPlan("profile-drift-target");
  const node = { id: "source81" };
  const base = {
    place: () => [{ node, score: 1 }],
    launch: (_p, n) => ({ ok: true, vm: { id: `vm-${n.id}` }, node: n }),
  };
  ...
  const sourceProfile = P.bind(sourcePlan, P.MACHINE_TYPE.CIRCUIT_ST81);
  const driftProfile = P.bind(driftPlan, P.MACHINE_TYPE.CIRCUIT_ST81);
  assert.equal(sourceProfile.machineType, driftProfile.machineType);
  assert.equal(sourceProfile.logicalDimension, driftProfile.logicalDimension);
  assert.notEqual(sourceProfile.digest, driftProfile.digest);
  const vm = engine.launch(sourcePlan, sourceProfile, node).vm;
  const contract = engine.compileGoMicroVM(sourcePlan, sourceProfile);
...
test("full checkpoint restore rejects same-shaped construction-profile drift", () => {
  const f = fixture();
  const gate = D.checkpointAdmission({
    sourcePassport: f.passport,
    targetProfile: f.driftProfile,
    kind: D.CHECKPOINT.FULL_RESTORE,
  });
  assert.equal(gate.ok, false);
  assert.equal(gate.code, "PROFILE_DRIFT_FULL_RESTORE_FORBIDDEN");
});

test("neutral continuation across same-shaped profile drift starts a new machine identity", () => {
  const f = fixture();
  const gate = D.checkpointAdmission({
    sourcePassport: f.passport,
    targetProfile: f.driftProfile,
  ...
  assert.equal(gate.code, "NEUTRAL_CONTINUATION_PROFILE_DRIFT");
  assert.equal(gate.startsNewMachineIdentity, true);
});
...
test("failover cannot disguise same-shaped profile drift as full restore", () => {
  ...
  const r = F.planRecovery({
    ...
    safePoint: "SYSCALL_BOUNDARY",
    neutralStateDigest: d("neutral"),
    candidates: [{ nodeId: "drift81", profile: f.driftProfile, estimatedRecoveryMs: 1 }],
  });
  assert.equal(r.ok, true);
  assert.equal(r.mode, D.CHECKPOINT.NEUTRAL_CONTINUATION);
  assert.equal(r.nodeId, "drift81");
