...
const fixturePath = process.env.W33_EXECUTION_FIXTURE;
if (!fixturePath) {
  test("cross-repo W33 execution fixture is supplied by the dedicated CI gate", { skip: true }, () => {});
} else {
  test("Holotrade owns select -> verifier -> real W33 execution -> signed delivery in one transaction", () => {
    const execution = JSON.parse(fs.readFileSync(fixturePath, "utf8"));
