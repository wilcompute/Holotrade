        ...
        with open(p, "w") as fh:
            json.dump({
                ...
                "whyPrimeOrderSuffices": ("two steps. Any automorphism of a "
                                          ...
                                          "indicative"),
                "instances": rows,
                "aStrengtheningThatDidNotHelp": ("the nine undecided order-3 "
                                                 "instances were re-run with the "
                                                 "global tile-occupancy identity "
                                                 "added explicitly: an integer "
                                                 "variable occ[L][M] in [1,16] "
                                                 "for each of the 1600 tiles, "
                                                 "equal to that tile's leaf "
                                                 "count, with sum(occ) = 16 x 111 "
                                                 "= 1776 (each leaf lies in four "
                                                 "row-lines and four "
                                                 "column-lines). The constraint "
                                                 "is redundant but gives the "
                                                 "solver a global counting handle "
                                                 "that plain >= 1 tile "
                                                 "constraints do not. It changed "
                                                 "nothing: all nine remained "
                                                 "UNKNOWN. Recorded so the next "
                                                 "attempt does not spend the same "
                                                 "budget on the same idea"),
