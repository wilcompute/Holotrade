    ...
    if "--write" in sys.argv:
        ...
        with open(p, "w") as fh:
            json.dump({
                ...
                "valid": bool(ok),
                "nowProvedElsewhere": ("this file VERIFIES the section at "
                                       "p = 3, 5, 7 for n = 1; it does not prove "
                                       "it. A proof now exists and supersedes "
                                       "the verification: Theory-of-Everything "
                                       "40a798487, analysis/w33_odd_prime_weyl_"
                                       "transvection_theorem.py, proves "
                                       "G D_x G^dag = D_(T_(v,lambda) x) with "
                                       "zero residual Weyl/Pauli phase for EVERY "
                                       "odd prime p and EVERY n, taking "
                                       "G = p^(-1/2) sum_k omega^(a k^2) D_(kv) "
                                       "with a = -(2 lambda)^(-1), by matching "
                                       "coefficients under -2 a lambda = 1 and "
                                       "h lambda + a lambda^2 = 0, and getting "
                                       "unitarity from the quadratic Gauss sum "
                                       "directly rather than from a "
                                       "classification theorem. Their h = 2^(-1) "
                                       "is the same phase this file measured as "
                                       "t = (p+1)/2 = 2^(-1). This commit is nine "
                                       "minutes earlier (21:03:44 against "
                                       "21:12:13 on 2026-09-04) so by the "
                                       "corpus ownership rule it owns the "
                                       "empirical result -- but ownership is not "
                                       "the point: the PROOF is theirs and is "
                                       "strictly stronger, covering every n and "
                                       "every odd prime rather than three primes "
                                       "at n = 1, and anyone quoting the "
                                       "p = 3,5,7 table below should quote the "
                                       "theorem instead"),
