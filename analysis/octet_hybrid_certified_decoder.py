...
Fallback: best strict descent among the single certified 1,080-move PSp(4,3)
orbit of the primitive circuit lying outside that radius-two ball.

The minimization certificate proves Orb(c)=Orb(-c), so no explicit sign-union is
needed. Every library move is independently verified to lie in ker_Z(N^T), and
the canonical move lists are content-addressed. Every accepted step strictly
lowers the nonnegative integer negativity objective, so every run terminates
after at most its starting negativity many accepted steps. No complete Graver
basis is needed at runtime.
...
MIN_CERT=HERE/'octet_circuit_library_minimization_certificate.json'
...
def main():
    ...
    mincert=json.loads(MIN_CERT.read_text()); assert mincert['status']=='PASS' and mincert['orientationOrbitRelation']=='same-orbit'
    assert mincert['minimalPSpInvariantCircuitMoveCount']==1080 and mincert['minimalSelectedOrientationOrbits']==['plus']
    point_gens=m24c.point_action_generators(pencils,line_gens)
    co=orbit_vectors(base,point_gens)
    negco={tuple(-v for v in x) for x in co}
    assert len(co)==1080 and co==negco
    circuits=tuple(sorted(co))
    circuit_digest=digest_moves(circuits)
    assert circuit_digest==mincert['minimalLibraryDigest']==mincert['orientationOrbitDigests']['plus']
    ...
    out={
      'schema':'holotrade.octet-hybrid-certified-decoder.v3','status':'PASS','group':'PSp(4,3)','groupOrder':25920,
      'radiusTwoMoveCount':len(radius2),'radiusTwoLibraryDigest':digest_moves(radius2),
      'circuitOrbitSize':len(co),'signedCircuitMoveCount':len(circuits),'orientationOrbitClosedUnderSign':True,
      'outsideRadiusTwoCircuitMoveCount':len(outside),'circuitLibraryDigest':circuit_digest,
      'minimalCircuitLibraryCertificate':MIN_CERT.name,'minimalPSpInvariantCircuitMoveCount':mincert['minimalPSpInvariantCircuitMoveCount'],
      ...
      'theorem':'The runtime fallback is exactly the certified minimal orientation-orbit library: one 1080-element PSp(4,3) orbit with Orb(c)=Orb(-c). Radius-two octet descent is attempted first, then that content-addressed orbit. The frozen mass-28 traps exercise the fallback and reach independently certified optimum.',
    ...
    print(json.dumps({k:out[k] for k in ['status','radiusTwoMoveCount','circuitOrbitSize','signedCircuitMoveCount','orientationOrbitClosedUnderSign','outsideRadiusTwoCircuitMoveCount','benchmarkStartCount','benchmarkReachedCertifiedOptimum','benchmarkStalled','fallbackCircuitStepsUsed','fallbackPathExercised']},indent=2,sort_keys=True)); print(f'written: {p}')
