...
const RTL=path.join(ROOT,"rtl","w33_216_typed_microvm.v");
...
function build(){
  ...
  const actions=fields(line(main,"MICROVM_ACTIONS"));
  ...
  for(let g=0;g<generatorCount;g++)for(let x=0;x<216;x++){
    ...
    if(hemisystemBlockIds[hemisystemGenerators[g][x]]!==quotientGenerators[g][hemisystemBlockIds[x]])throw new Error("hemisystem quotient equivariance drift");
  }

  const body={
    schema:"holotrade.w33-five-front-breakthrough.v1",
    ...
    steinbergIntertwiner:{
      ...
      projector:{degree:Number(projector.degree),characterIndex:Number(projector.characterIndex),characterValues:ints(projector.characterValues),rank:Number(projector.rank),idempotent:projector.idempotent==="1"},
    ...
    microvm:{
      statesPerCarrier:216,carrierTypes:["sentinel-circuit","hemisystem"],generatorCount,
...
function main(){
  ...
  process.stdout.write(JSON.stringify({status:"PASS",steinbergRank:packet.steinbergIntertwiner.rank,quotient:packet.twoS5Correspondence.minimumRelation.quotientDegree,residualParity:packet.colourSwap.residual15Parity,maxTriple:packet.tau111Defect.maximumTripleFibres,microvm:[2,216,36],sha256:packet.sha256})+"\n");
}
