...
(function(root){
  ...
  // Blocking numbers are SUBMULTIPLICATIVE: if X blocks every depth-a tile
  // and Y every depth-b tile, then X x Y blocks every depth-(a+b) tile,
  // because a tile L_1 x ... x L_(a+b) splits into its first a and last b
  // factors and each half is met independently.  So tau_(a+b) <= tau_a*tau_b.
  // With tau_1 = 11 and tau_2 <= 115 the best product at depth n uses as many
  // depth-2 factors as possible: 115^floor(n/2) * 11^(n mod 2), which beats
  // 11^n at every n >= 2 (13,225 against 14,641 at n = 4).
  function bestProduct(depth){const pairs=Math.floor(depth/2),odd=depth%2;return Math.pow(SYMMETRIC_WITNESS.length,pairs)*Math.pow(TAU1,odd);}
  function tensorBlockingBounds(depth=2){if(!Number.isInteger(depth)||depth<1)throw new RangeError("depth must be positive integer");const lower=TAU1*Math.pow(10,depth-1),product=Math.pow(TAU1,depth),upper=depth===1?TAU1:bestProduct(depth);return Object.freeze({depth,lower,upper,productUpper:product,upperBeatsProduct:upper<product,exact:depth===1,proof:depth===1?"repository SAT certificate":"Recursive shadow double-count: each fixed first-axis line needs a depth-(n-1) blocker, 40 line shadows are counted, and each leaf belongs to 4 first-axis lines, multiplying the lower bound by 10. Upper bound at depth 2 is the explicit 115-leaf order-6-invariant blocker; at higher depths it is the submultiplicative product 115^floor(n/2)*11^(n mod 2), which strictly beats the naive B^n for every n >= 2.",boundary:"Finite-combinatorial placement guarantee only; not a physical availability SLA."});}
  ...
  const API={TAU1,BLOCKER,SYMMETRIC_WITNESS,bestProduct,lineBlockerCertificate,productTile,tensorBlockingBounds,depth2Certificate,pairedSpreadShard,recursiveCapacity,hitsAllProductTiles};root.HolotradeTensorSharding=API;if(typeof module!=="undefined"&&module.exports)module.exports=API;
